...
sources for executing the SQL and processing any results (largely not shared code)
...
[NOTE]
====
Discussing with Christian, we should have a way to send arbitrary "batch blocks" to the database
and process arbitrary result counts/sets much like our procedure call execution/result-handling does.
====
...
The design here is too abstract access to the JDBC results as
