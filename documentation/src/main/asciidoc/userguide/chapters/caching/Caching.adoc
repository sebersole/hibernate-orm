...
[NOTE]
====
Use of the build-in integration for https://jcp.org/en/jsr/detail?id=107[JCache] requires that the `hibernate-orm-jcache` module jar (and all of its dependencies) are on the classpath.
...
The `hibernate-orm-jcache` module defines the following region factory: `JCacheRegionFactory`.
...
The hibernate-orm-infinispan module defines two specific providers: `infinispan` and  `infinispan-jndi`.
