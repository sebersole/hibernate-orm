...
`org.hibernate.envers.use_numeric_revend_timestamp_type` (default `false`)::
When enabled, uses `Long` data type to represent the revision end timestamp value rather than `Timestamp`.
...
`org.hibernate.envers.audit_strategy_validity_legacy_revend_timestamp` (dfault: `true`)::
Should use legacy validity-based audit strategy's joined inheritance-based revision end timestamp behavior.
...
.  `org.hibernate.envers.audit_strategy_validity_legacy_revend_timestamp` (See HHH-9062)
====
...
revision timestamp::
Either a `long/Long`, `java.util.Date`, or `java.time.LocalDateTime` value representing the instant at which the revision was made.
...
`org.hibernate.envers.audit_strategy_validity_legacy_revend_timestamp`
