...
Rather than embed OSGi capabilities into hibernate-orm, and sub-modules, hibernate-orm-osgi was created.
...
This is included in the binary distribution, as well as deployed to the JBoss Nexus repository (using the `org.hibernate.orm` groupId and `hibernate-orm-osgi` with the `karaf.xml` classifier).
...
hibernate-orm-osgi, theoretically, supports a variety of OSGi containers, such as Equinox.
...
It uses the JPA provider (hibernate-orm-osgi) that has registered itself with the OSGi `PersistenceProvider` service.
...
`hibernate-orm-osgi` registers an OSGi service, using the JPA `PersistenceProvider` interface name, that bootstraps and creates an `EntityManagerFactory` specific for OSGi environments.
...
`hibernate-orm-osgi` registers an OSGi service, using the `SessionFactory` interface name, that bootstraps and creates a `SessionFactory` specific for OSGi environments.
...
`hibernate-orm-osgi` supports the same extensions through OSGi services.
Implement and register them in any of the three configurations.
`hibernate-orm-osgi` will discover and integrate them during `EntityManagerFactory`/`SessionFactory` bootstrapping. Supported extension points are as follows.
