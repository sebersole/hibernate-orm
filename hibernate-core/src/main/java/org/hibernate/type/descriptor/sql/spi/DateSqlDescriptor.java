...
public class DateSqlDescriptor extends AbstractTemplateSqlTypeDescriptor implements TemporalSqlDescriptor {
	...
	protected <X> JdbcValueBinder<X> createBinder(BasicJavaDescriptor<X> javaTypeDescriptor) {
		return new AbstractJdbcValueBinder<X>(
				javaTypeDescriptor,
				this
		) {
			@Override
			protected void doBind(
				...
				final Date date = javaTypeDescriptor.unwrap(
						value,
						Date.class,
		...
		return new AbstractJdbcValueExtractor<X>( javaTypeDescriptor, this ) {
			...
			protected X doExtract(
				...
				return javaTypeDescriptor.wrap(
						rs.getDate( sqlSelection.getJdbcResultSetIndex() ),
			...
			protected X doExtract(
					...
					JdbcValuesSourceProcessingState processingState) throws SQLException {
				return javaTypeDescriptor.wrap(
						statement.getDate( sqlSelection.getJdbcResultSetIndex() ),
						processingState.getSession()
				);
			}
			...
			protected X doExtract(
					...
					String name,
					JdbcValuesSourceProcessingState processingState) throws SQLException {
				return javaTypeDescriptor.wrap( statement.getDate( name ), processingState.getSession() );
			}
		};
	}
}
