		...
		return new AbstractJdbcValueExtractor<X>( javaTypeDescriptor, this ) {
			...
			protected X doExtract(
				...
				return javaTypeDescriptor.wrap(
						rs.getShort( sqlSelection.getJdbcResultSetIndex() ),
			...
			protected X doExtract(
					...
					JdbcValuesSourceProcessingState processingState) throws SQLException {
				return javaTypeDescriptor.wrap(
						statement.getShort( sqlSelection.getJdbcResultSetIndex() ),
						processingState.getSession()
				);
			}
			...
			protected X doExtract(
					...
					String name,
					JdbcValuesSourceProcessingState processingState) throws SQLException {
				return javaTypeDescriptor.wrap( statement.getShort( name ), processingState.getSession() );
			}
		};
	}
}
