...
//	}
	...
	public void scope(SessionFactoryImplementor factory) {
		...
		for ( final PersistentClass model : mappingMetadata.getEntityBindings() ) {
			final EntityStorageAccess accessStrategy = getSessionFactory().getCache().determineEntityRegionAccessStrategy(
					model
			);

			final NaturalIdStorageAccess naturalIdAccessStrategy = getSessionFactory().getCache().determineNaturalIdRegionAccessStrategy(
					model
			);
