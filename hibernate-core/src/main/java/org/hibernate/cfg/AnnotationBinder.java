...
public final class AnnotationBinder {
	...
	public static void bindDefaults(MetadataBuildingContext context) {
		...
		{
			TableGenerators anns = (TableGenerators) defaults.get( TableGenerators.class );
			if ( anns != null ) {
				for ( TableGenerator ann : anns.value() ) {
					IdentifierGeneratorDefinition idGen = buildIdGenerator( ann, context );
					if ( idGen != null ) {
						context.getMetadataCollector().addDefaultIdentifierGenerator( idGen );
					}
		...
		{
			SequenceGenerators anns = (SequenceGenerators) defaults.get( SequenceGenerators.class );
			if ( anns != null ) {
				for ( SequenceGenerator ann : anns.value() ) {
					IdentifierGeneratorDefinition idGen = buildIdGenerator( ann, context );
					if ( idGen != null ) {
						context.getMetadataCollector().addDefaultIdentifierGenerator( idGen );
					}
