...
import org.hibernate.boot.annotations.parts.spi.AssociationOverrideMetadata;
import org.hibernate.boot.annotations.parts.spi.AttributeOverrideMetadata;
import org.hibernate.boot.annotations.parts.spi.ConversionMetadata;
...
import org.hibernate.boot.annotations.spi.JpaCallbackInformation;
import org.hibernate.boot.annotations.spi.MetadataHelper;
...
public abstract class AbstractIdentifiableTypeMetadata
	...
	private List<JpaCallbackInformation> collectedJpaCallbackSources;
	 ...
	 *
	 * @param managedClass The Entity/MappedSuperclass class descriptor
	 * @param defaultAccessType The default AccessType for the hierarchy
	 * @param isRootEntity Is this the root entity?
	...
	public AbstractIdentifiableTypeMetadata(
		...
		super( managedClass, defaultAccessType, isRootEntity, bindingContext );

		// walk up
		this.superType = walkRootSuperclasses( managedClass, defaultAccessType );
		if ( superType != null ) {
		...
		if ( isRootEntity ) {
			// walk down
			walkSubclasses( managedClass, this, defaultAccessType );
		}
	...
	private AbstractIdentifiableTypeMetadata walkRootSuperclasses(
			ManagedClass managedClass,
			AccessType defaultAccessType) {
		final ManagedClass superTypeManagedClass = managedClass.getSuperType();
		if ( superTypeManagedClass == null ) {
			return null;
		}

		// make triple sure there is no @Entity annotation
		if ( isEntity( superTypeManagedClass ) ) {
			throw new MappingException(
					String.format(
							Locale.ENGLISH,
							"Unexpected @Entity [%s] as MappedSuperclass of entity hierarchy",
							superTypeManagedClass.getName()
					),
					new Origin( SourceType.ANNOTATION, superTypeManagedClass.getName().toString() )
			);
		}
		else if ( isMappedSuperclass( superTypeManagedClass ) ) {
			return new MappedSuperclassTypeMetadataImpl(
					superTypeManagedClass,
					walkRootSuperclasses( superTypeManagedClass, defaultAccessType ),
					defaultAccessType,
			...
			if ( superTypeManagedClass.getSuperType() != null ) {
				return walkRootSuperclasses( superTypeManagedClass, defaultAccessType );
			}
			else {
				return null;
			}
	...
	protected boolean isMappedSuperclass(ManagedClass managedClass) {
		return managedClass.getAnnotation( JpaAnnotations.MAPPED_SUPERCLASS ) != null;
	}

	protected boolean isEntity(ManagedClass managedClass) {
		return managedClass.getAnnotation( JpaAnnotations.ENTITY ) != null;
	}
	...
	private void walkSubclasses(
			...
			AbstractIdentifiableTypeMetadata superType,
			AccessType defaultAccessType) {
		getLocalBindingContext().getManagedClassRegistry().forEachDirectSubType( managedClass.getName(), (subTypeManagedClass) -> {
	 ...
	 */
	public List<JpaCallbackInformation> getJpaCallbacks() {
	...
	public void forEachJpaCallback(Consumer<JpaCallbackInformation> consumer) {
		getJpaCallbacks().forEach( consumer );
	}

	private List<JpaCallbackInformation> collectJpaCallbacks() {
		...
		// collection
		final ArrayList<JpaCallbackInformation> callbacks = new ArrayList<>();
...
//			}
	...
	private void collectEntityListenerCallbacks(
			...
			ArrayList<JpaCallbackInformation> callbacks) {
		...
		if ( entityListenersAnnotation != null ) {
			final ManagedClass[] listenerClasses = entityListenersAnnotation.getValueAttributeValue().getValue();
