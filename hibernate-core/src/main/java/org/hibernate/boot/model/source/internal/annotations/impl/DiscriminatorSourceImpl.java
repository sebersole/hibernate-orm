 ...
 */
public class DiscriminatorSourceImpl extends DiscriminatorSourceSupport {
	...
	public DiscriminatorSourceImpl(EntityTypeMetadata entityTypeMetadata) {
		...
		this.entityName = entityTypeMetadata.getName();

		final EntityBindingContext bindingContext = entityTypeMetadata.getLocalBindingContext();
		final AnnotationInstance discriminatorColumn = entityTypeMetadata.findTypeAnnotation( JpaDotNames.DISCRIMINATOR_COLUMN );
		final AnnotationInstance discriminatorFormula = entityTypeMetadata.findTypeAnnotation( HibernateDotNames.DISCRIMINATOR_FORMULA );
		...
		if ( discriminatorColumn != null && discriminatorFormula != null ) {
			...
			if ( columnNameValue != null ) {
				throw entityTypeMetadata.getLocalBindingContext().makeMappingException(
						"Entity hierarchy [" + entityTypeMetadata.getName() +
		...
		if ( isFormula ) {
			final String expression = bindingContext.getTypedValueExtractor( String.class )
					.extract( discriminatorFormula, "value" );
			this.relationalValueSource = new DerivedValueSourceImpl( expression, null );
		}
		else {
			discriminatorType = bindingContext.getTypedValueExtractor( DiscriminatorType.class )
					.extract( discriminatorColumn, "discriminatorType" );

			final Column column = new Column( null );
			column.setNullable( false );
			column.setName(
					bindingContext.getTypedValueExtractor( String.class )
							.extract( discriminatorColumn, "name" )
			);
			column.setLength(
					bindingContext.getTypedValueExtractor( Integer.class )
