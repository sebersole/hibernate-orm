...
public class EntityHierarchySourceImpl implements EntityHierarchySource {
	...
	private IdentifierSource determineIdentifierSource(RootEntityTypeMetadata root, RootEntitySourceImpl rootSource) {
		...
		switch ( idType ) {
			case SIMPLE: {
				return new IdentifierSourceSimpleImpl(
						rootSource,
						(SingularAttributeSourceBasic) rootSource.getIdentifierAttributes().get( 0 )
				);
			}
			case AGGREGATED: {
				return new IdentifierSourceCompositeAggregatedImpl(
						rootSource,
						(SingularAttributeSourceEmbedded) rootSource.getIdentifierAttributes().get( 0 ),
						rootSource.getMapsIdSources()
				);
			}
			case NON_AGGREGATED: {
				return new IdentifierSourceCompositeNonAggregatedImpl( rootSource );
			}
			default: {
