...
public class ToOneBinder {
	...
	public static void bindForeignKeyNameAndDefinition(
		...
		if ( property.hasAnnotationUsage( NotFound.class ) ) {
			...
			final AnnotationUsage<JoinColumn> joinColumn = property.getSingleAnnotationUsage( JoinColumn.class );
			final AnnotationUsage<JoinColumns> joinColumns = property.getAnnotationUsage( JoinColumns.class );
			if ( joinColumn!=null && noConstraint( joinColumn.getNestedUsage( "foreignKey" ), context )
					|| joinColumns!=null && noConstraint( joinColumns.getNestedUsage( "foreignKey" ), context ) ) {
				value.disableForeignKey();
			}
			else {
	...
	private static boolean noConstraint(AnnotationUsage<ForeignKey> fkAnn, MetadataBuildingContext context) {
		if ( fkAnn == null ) {
			return false;
		}
		else {
			final ConstraintMode mode = fkAnn.getEnum( "value" );
