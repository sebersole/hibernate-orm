...
public class DefaultCatalogAndSchemaTest {
	...
	private void verifyEntityPersisterQualifiers(Class<?> entityClass, ExpectedQualifier expectedQualifier) {
		...
		final String[] insertSqls = new String[persister.getStaticSqlInsertGroup().getNumberOfTableMutations()];
		persister.getStaticSqlInsertGroup().forEachTableMutation( (tablePosition, tableInsert) -> {
			insertSqls[tablePosition] = tableInsert.getSqlString();
		} );
